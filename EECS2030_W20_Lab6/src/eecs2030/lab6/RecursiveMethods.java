...
public class RecursiveMethods {
	...
	public String repeatCharNTimes(int n, char c) {
		...
		StringBuilder sb;
		if(n > 0){
			sb = new StringBuilder(repeatCharNTimes(n-1, c));
			sb.insert(0, c);
		} else {
	...
	public int numberOfFirstChar0(String str) {
		...
		if (str.length()>0){
			...
			result = numberOfFirstChar0(firstChar, str.substring(1),1);
		}
		return result;
	}
	
	private int numberOfFirstChar0(char c, String str, int count){
		if(str.length() > 0){
			if(c == str.charAt(0)){
			count++;
			}
			count = numberOfFirstChar0(c, str.substring(1), count);
		}
		return count;
	}
	...
	public int numberOfFirstChar(String str) {
		...
		if(str.length() > 0){
			char c = str.charAt(0);
			result = numberOfFirstChar(c, str);
		}
		return result;
	}

	private int numberOfFirstChar(char c, String str){
		int result = 0;
		if(str.length() > 0){
			if(c == str.charAt(0)){
				result++;
				result += numberOfFirstChar(c, str.substring(1));
			}
		}
		return result;
	}
	...
	public char maxChar (String s) {
		// COMPLETE THIS
		// make this method recursive, or, call a recursive helper function
		return maxChar(s, s.charAt(0));
	}
	private char maxChar (String s, char c){
		char result = c;
		if(s.length() > 0){
			char nextChar = s.charAt(0);
			char maxChar = (c > nextChar) ? c : nextChar;
			result = maxChar(s.substring(1), maxChar); 
		}
		return result;
	}
	...
	public int []  IntegerToBinary (int n) {
		...
		IntegerToBinaryHelper(n, numberOfBits, a);
		return a;
	}
	private int getNumberOfBits(int n){
		...
		if(n<2){
			...
			result++;
			result += getNumberOfBits(n/2); 
		}
		return result;
	}
	private void IntegerToBinaryHelper(int n, int bit, int[] arr){
		int index = bit - 1;
		if(index >= 0){
			arr[index] = n % 2;
			IntegerToBinaryHelper(n/2, --bit, arr);
		}
	}
	 ...
	 */
	 public boolean isArithmeticArray(int[] a) {
		...
		boolean result = true;
		if(a.length>1){
			int seedDiff = a[0] - a[1];
			result = isArithmeticArrayHelper(a, seedDiff);
		}
		return result;
	}
	private boolean isArithmeticArrayHelper(int[] a, int diff){
		int[] newArr = new int [a.length - 1];
		System.arraycopy(a, 1, newArr, 0, a.length - 1);
		if(newArr.length>1){
			if(newArr[0] - newArr[1] == diff){
				if(isArithmeticArrayHelper(newArr, diff)){
					return true;
				}
			}
			return false;
